"""Group 6: Sampling Full Truncated (Mixture + HMM)

Tests full model with both mixture and HMM enabled.
Config: use_mixture=True, use_directional_hmm=True
Truncated to max_T=80

Usage:
    pixi run python test_group_6_sampling_full_truncated.py --run_id full_test
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Group 6: Sampling Full Truncated")
    parser.add_argument(
...
def build_model(data):
    """Build full model with mixture and HMM."""
    ...
    # Build model with mixture and HMM
    with pm.Model() as model:
        gimbal.build_camera_observation_model(
            ...
            image_size=(1280, 720),
            use_directional_hmm=False,  # Build without HMM first
...
def sample_model(model, chains, tune, draws, seed):
    ...
    summary = az.summary(
        trace, var_names=["rho", "sigma2", "logodds_inlier", "dir_hmm_kappa"]
    )
...
def generate_plots(trace, paths):
    ...
    try:
        ...
        az.plot_trace(
            trace, var_names=["rho", "logodds_inlier", "dir_hmm_kappa"], axes=axes
        )
        fig.tight_layout()
...
def run_group_6(args):
    """Run Group 6 diagnostics."""
    if args.run_id is None:
        args.run_id = datetime.now().strftime("%Y%m%d_%H%M%S")

    paths = make_paths("group_6_sampling_full_truncated", args.run_id)

    results = {
        "group": "group_6_sampling_full_truncated",
        "run_id": args.run_id,
...
def generate_report(results, paths):
    ...
    if "sampling_diagnostics" in results:
        ...
        if div_rate == 0:
            lines.append("**STABLE**: Full model converges successfully.")
        elif div_rate < 0.1:
            lines.append("**MOSTLY STABLE**: Full model has minor issues.")
        else:
            lines.append(
                "**UNSTABLE**: Interaction between mixture and HMM causes divergences."
            )
