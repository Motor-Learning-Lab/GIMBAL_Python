"""Group 5: Sampling Directional HMM Only

Tests directional HMM prior without mixture.
Config: use_mixture=False, use_directional_hmm=True
Truncated to max_T=80

Usage:
    pixi run python test_group_5_sampling_hmm_only.py --run_id hmm_test
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Group 5: Sampling HMM Only")
    parser.add_argument(
...
def build_model(data):
    """Build model with HMM but no mixture."""
...
def sample_model(model, chains, tune, draws, seed):
    ...
    summary = az.summary(trace, var_names=["rho", "sigma2", "dir_hmm_kappa"])
...
def generate_plots(trace, paths):
    ...
    try:
        ...
        az.plot_trace(trace, var_names=["rho", "dir_hmm_kappa"], axes=axes)
        fig.tight_layout()
...
def run_group_5(args):
    """Run Group 5 diagnostics."""
    if args.run_id is None:
        args.run_id = datetime.now().strftime("%Y%m%d_%H%M%S")

    paths = make_paths("group_5_sampling_hmm_only", args.run_id)

    results = {
        "group": "group_5_sampling_hmm_only",
        "run_id": args.run_id,
    ...
    try:
        ...
        print("\nBuilding model with HMM (no mixture)...")
...
def generate_report(results, paths):
    ...
    if "sampling_diagnostics" in results:
        ...
        if div_rate == 0:
            lines.append(
                "**STABLE WITH HMM**: Directional HMM prior converges successfully."
            )
        elif div_rate < 0.1:
            lines.append("**MOSTLY STABLE**: HMM introduces minor issues.")
        else:
            lines.append(
                "**UNSTABLE WITH HMM**: HMM Op gradients or parameterization causes divergences."
            )
