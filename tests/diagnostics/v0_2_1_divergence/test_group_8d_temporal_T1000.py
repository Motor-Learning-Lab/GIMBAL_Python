"""Group 8d: Temporal Scaling Test (T=1000)

Tests full model (mixture + HMM) at T=80 frames.
Baseline for temporal scaling analysis.

Usage:
    pixi run python test_group_8d_temporal_T1000.py --run_id T1000_test
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Group 8d: Temporal Scaling T=80")
    parser.add_argument(
...
def run_group_8a(args):
    """Run Group 8d diagnostics."""
    MAX_T = 1000

    if args.run_id is None:
        args.run_id = datetime.now().strftime("%Y%m%d_%H%M%S")

    paths = make_paths("group_8d_temporal_T1000", args.run_id)

    results = {
        "group": "group_8d_temporal_T1000",
        "run_id": args.run_id,
    ...
    try:
        ...
        print(f"\n  Sampling complete:")
        print(
            f"    Divergences: {sampling_diagnostics['divergences']}/{sampling_diagnostics['total_samples']} ({sampling_diagnostics['divergence_rate']:.1%})"
        )
        print(
            f"    Time: {sampling_diagnostics['elapsed_seconds']:.1f}s ({sampling_diagnostics['draws_per_second']:.2f} draws/s)"
        )
...
def generate_report(results, paths):
    ...
    if "sampling_diagnostics" in results:
        ...
        if div_rate == 0:
            lines.append(
                "**BASELINE STABLE** at T=80. Full model converges successfully."
            )
        elif div_rate < 0.1:
            lines.append(f"**MOSTLY STABLE** at T=80 ({div_rate:.1%} divergence rate).")
        else:
            lines.append(
                f"**UNSTABLE** at T=80 ({div_rate:.1%} divergence rate) - Issue not temporal scaling."
            )
