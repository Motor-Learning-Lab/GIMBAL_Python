"""Group 8a: Temporal Scaling Test (T=80)

Tests full model (mixture + HMM) at T=80 frames.
Baseline for temporal scaling analysis.

Usage:
    pixi run python test_group_8a_temporal_T80.py --run_id T80_test
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Group 8a: Temporal Scaling T=80")
    parser.add_argument(
...
def run_group_8a(args):
    """Run Group 8a diagnostics."""
    MAX_T = 80

    if args.run_id is None:
        args.run_id = datetime.now().strftime("%Y%m%d_%H%M%S")

    paths = make_paths("group_8a_temporal_T80", args.run_id)

    results = {
        "group": "group_8a_temporal_T80",
        "run_id": args.run_id,
