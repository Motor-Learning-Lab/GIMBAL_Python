...
def run_geometry_analysis():
    ...
    print("  Note: Small draws to complete test quickly; diagnostics focus on divergences")
    ...
    print(f"  Divergence rate: {100.0 * trace.sample_stats.diverging.sum().values / len(trace.posterior.draw):.1f}%")
    print()
    ...
    try:
        ...
        plt.tight_layout()
        fig.savefig(diagnostics_dir / f"{test_name}_trace_key_params.png", dpi=150, bbox_inches='tight')
        plt.close(fig)
    ...
    try:
        ...
        fig.savefig(diagnostics_dir / f"{test_name}_energy.png", dpi=150, bbox_inches='tight')
        plt.close(fig)
    except Exception as e:
    ...
    try:
        ...
        fig.savefig(diagnostics_dir / f"{test_name}_parallel.png", dpi=150, bbox_inches='tight')
        plt.close(fig)
    except Exception as e:
    ...
    try:
        ...
        logodds = posterior['logodds_inlier'].values if 'logodds_inlier' in posterior else np.zeros_like(eta2_root)
        diverging = trace.sample_stats.diverging.values.flatten()
        
        # eta2_root vs obs_sigma
        axes[0, 0].scatter(eta2_root[~diverging], obs_sigma[~diverging], alpha=0.5, s=30, label='Non-divergent', color='blue')
        axes[0, 0].scatter(eta2_root[diverging], obs_sigma[diverging], alpha=0.7, s=50, label='Divergent', color='red', marker='x')
        ...
        axes[0, 1].scatter(np.where(diverging)[0], eta2_root[diverging], color='red', s=50, marker='x', label='Divergences', zorder=5)
        ...
        axes[1, 0].scatter(np.where(diverging)[0], obs_sigma[diverging], color='red', s=50, marker='x', label='Divergences', zorder=5)
        ...
        axes[1, 1].hist([eta2_root[~diverging], eta2_root[diverging]], 
                        label=['Non-divergent', 'Divergent'], bins=10, alpha=0.6, color=['blue', 'red'])
        ...
        plt.tight_layout()
        fig.savefig(diagnostics_dir / f"{test_name}_posterior_scatter.png", dpi=150, bbox_inches='tight')
        plt.close(fig)
    except Exception as e:
    ...
    try:
        ...
        axes[0].barh(vars_to_plot, rhat_values, color=['red' if v > 1.05 else 'blue' for v in rhat_values])
        ...
        plt.tight_layout()
        fig.savefig(diagnostics_dir / f"{test_name}_diagnostics_rhat_ess.png", dpi=150, bbox_inches='tight')
        plt.close(fig)
    except Exception as e:
    ...
    # Save results
    results_file = Path(f"results_{test_name}.json")
