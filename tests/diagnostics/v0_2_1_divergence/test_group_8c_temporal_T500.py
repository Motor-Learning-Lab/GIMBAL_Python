"""Group 8c: Temporal Scaling Test (T=500)

Tests full model (mixture + HMM) at T=80 frames.
Baseline for temporal scaling analysis.

Usage:
    pixi run python test_group_8c_temporal_T500.py --run_id T500_test
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Group 8c: Temporal Scaling T=80")
    parser.add_argument(
...
def run_group_8a(args):
    """Run Group 8c diagnostics."""
    MAX_T = 500

    if args.run_id is None:
        args.run_id = datetime.now().strftime("%Y%m%d_%H%M%S")

    paths = make_paths("group_8c_temporal_T500", args.run_id)

    results = {
        "group": "group_8c_temporal_T500",
        "run_id": args.run_id,
