"""Group 8b: Temporal Scaling Test (T=200)

Tests full model (mixture + HMM) at T=80 frames.
Baseline for temporal scaling analysis.

Usage:
    pixi run python test_group_8b_temporal_T200.py --run_id T200_test
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Group 8b: Temporal Scaling T=80")
    parser.add_argument(
...
def run_group_8a(args):
    """Run Group 8b diagnostics."""
    MAX_T = 200

    if args.run_id is None:
        args.run_id = datetime.now().strftime("%Y%m%d_%H%M%S")

    paths = make_paths("group_8b_temporal_T200", args.run_id)

    results = {
        "group": "group_8b_temporal_T200",
        "run_id": args.run_id,
