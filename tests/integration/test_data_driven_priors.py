...
def test_triangulate_multi_view_basic():
    ...
    positions_3d = gp.cameras.triangulation.triangulate_multi_view(
...
def test_triangulate_multi_view_insufficient_cameras():
    ...
    result = gp_tri.triangulate_multi_view(keypoints_2d, camera_proj, min_cameras=2)
...
def test_clean_keypoints_2d_outlier_detection():
    ...
    keypoints_clean, valid_mask, summary = gp_clean.clean_keypoints_2d(
...
def test_clean_keypoints_2d_interpolation():
    ...
    parents = np.array([-1])
    config = gp_clean.CleaningConfig(max_gap=5)
...
def test_clean_keypoints_3d_statistics_mask():
    ...
    config = gp_clean.CleaningConfig(jump_z_thresh=3.0, max_gap=2)

    positions_clean, valid_mask, use_for_stats, summary = gp_clean.clean_keypoints_3d(
...
def test_build_priors_from_statistics():
    ...
    prior_config = gp_priors.build_priors_from_statistics(
...
def test_gamma_mode_sd_to_shape_rate():
    """Test Gamma parameterization conversion."""
    mode = 2.0
    sd = 1.0

    shape, rate = gp_priors.gamma_mode_sd_to_shape_rate(mode, sd)
...
def test_integration_full_pipeline():
    ...
    # Use demo skeleton
    skeleton = gp_config.DEMO_V0_1_SKELETON
    ...
    # Step 1: Clean 2D
    config = gp_clean.CleaningConfig()
    kp_2d_clean, valid_2d, summary_2d = gp_clean.clean_keypoints_2d(
        keypoints_2d, skeleton.parents, config
    )
    assert kp_2d_clean.shape == keypoints_2d.shape

    # Step 2: Triangulate
    positions_3d_tri = gp_tri.triangulate_multi_view(kp_2d_clean, camera_proj)
    assert positions_3d_tri.shape == (T, K_full, 3)

    # Step 3: Clean 3D
    pos_3d_clean, valid_3d, use_for_stats, summary_3d = gp_clean.clean_keypoints_3d(
