# GIMBAL v0.2.1: Systematic Divergence Testing Plan

> **📋 Before starting:** Read [`IMPORTANT_FILE_NAMING_CONVENTIONS.md`](../IMPORTANT_FILE_NAMING_CONVENTIONS.md) in the repository root for file organization requirements.
...
- If divergences increase substantially → HMM is problematic
...
**Interpretation:**
- If divergences drop substantially with more tuning → Model just needs better adaptation
- If divergences persist regardless of tuning → Fundamental posterior geometry issue
- If divergences only in tuning phase → Can safely ignore (tuning samples are discarded)
- If divergences only in sampling phase → Model is poorly adapted despite tuning
...
**Interpretation:**
- If divergences decrease with fewer states → Multi-state symmetry/label-switching is an issue
- If divergences similar across state counts → State count is not the problem
...
**Interpretation:**
- Balanced ratio (0.3 to 3.0) → Both components contribute appropriately
- Imbalanced ratio → Dominant component may drive pathological geometry
...
**Interpretation:**
- Clustered divergences in specific parameter regions → Funnel geometry
- Divergences scattered across parameter space → HMM exploration issues
- High correlation with specific parameters → Targeted reparameterization needed
...
**Interpretation:**
- Divergences decrease with relaxed prior → Funnel geometry confirmed
- Divergences unchanged → Root variance is not the issue
...
**Interpretation:**
- Divergences decrease when `sigma2` fixed → Hierarchical structure is problematic
- Divergences unchanged → Bone-length variance is not the issue
...
**Interpretation:**
- Unexpected scaling patterns → Implementation issue in collapsed HMM
- Consistent scaling with correct gradients → Implementation is sound
...
├── test_group_1_baseline_no_hmm.py          # Test implementation
├── test_group_2_baseline_with_hmm.py        # Test implementation
├── test_group_3_state_count.py              # Test implementation
├── ...
├── results_group_1_baseline_no_hmm.json     # Raw output data
├── results_group_2_baseline_with_hmm.json   # Raw output data
├── report_group_1_baseline_no_hmm.md        # Analysis and interpretation
├── report_group_2_baseline_with_hmm.md      # Analysis and interpretation
└── report_summary.md                        # Overall findings across all tests
```
