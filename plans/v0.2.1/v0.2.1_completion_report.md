...
4. **⚠️ CRITICAL - Initialization Bug (2025-12-10)**: `inlier_prob` initialization could be exactly 1.0, causing `logodds_init = inf` and model initialization failure
   - **Root cause**: `fit_params._estimate_observation_parameters_numpy()` line 448 computes `inlier_prob = inlier_mask.sum() / len(errors)`, which equals 1.0 when all observations are inliers
   - **Symptom**: Divergence tests all failed with `-inf` log probability at initialization, preventing any sampling
   - **Fix**: Added `np.clip(inlier_prob, 0.01, 0.99)` to respect data while avoiding boundary issues in logodds transformation
   - **Impact**: All divergence tests now initialize successfully (log probability `-24015` instead of `-inf`)
   - **Files modified**: `gimbal/fit_params.py` line 448-450
   - **Test validation**: `tests/diagnostics/v0_2_1_divergence/test_ground_truth_init.py` confirms fix
...
- HMM sampling produces divergences (200/200 in demo) - inherent to HMM posterior geometry, not a bug (requires further investigation with divergence test suite)
...
## Post-Release Status Update (2025-12-10)

### Critical Bug Fixed: Model Initialization

**Problem Discovered:**
The comprehensive divergence test suite (`tests/diagnostics/v0_2_1_divergence/`) revealed that ALL tests were failing at model initialization, preventing any divergence analysis from occurring.

**Diagnosis Process:**

1. Created `test_ground_truth_init.py` to isolate initialization from sampling issues
2. Implemented `extract_ground_truth.py` to extract complete parameter sets (42 parameters)
3. Developed `debug_model_logp.py` for systematic per-parameter debugging
4. Identified: `logodds_inlier = inf` when `inlier_prob_init = 1.0`

**Root Cause:**

- `fit_params._estimate_observation_parameters_numpy()` computes inlier probability from reprojection errors
- When all observations are inliers (perfect data or low threshold), `inlier_prob = 1.0` exactly
- Logodds transformation: `log(1.0 / 0.0) = inf` causes model to reject initialization

**Solution Implemented:**

```python
# gimbal/fit_params.py line 448-450
...
- Prior: `Normal(mu=0, sigma=5)` on logodds scale, centered at p=0.5
- Clip range `[0.01, 0.99]` corresponds to logodds `[-4.6, 4.6]` (within prior support)
- Test result: Default initialization now gives finite log probability (`-24015` vs previous `-inf`)
- All divergence diagnostic tools now operational

### Current Status: Ready for Divergence Analysis

**Completed Infrastructure:**

- ✅ Full divergence test suite (8 test groups)
- ✅ Ground truth initialization test
- ✅ Per-parameter debugging tools
- ✅ Comprehensive extraction/verification utilities
- ✅ Model initialization bug fixed

**Next Steps:**

1. **Run divergence test suite** (`tests/diagnostics/v0_2_1_divergence/test_runner.py`)
   - Now that initialization works, can actually measure divergence rates
   - Compare baseline (no HMM) vs HMM-enabled models
   - Identify which components contribute to divergences

2. **Analyze results** using `tests/diagnostics/v0_2_1_divergence/README.md`
   - Review generated diagnostic plots
   - Interpret divergence patterns
   - Determine if divergences are problematic or acceptable

3. **Consider model improvements** if needed (v0.2.2+)
   - Reparameterizations to improve posterior geometry
   - Prior adjustments based on empirical findings
   - Alternative HMM implementations if current causes issues

**Documentation Status:**

- ✅ `v0.2.1-completion-report.md` (this document) - updated with bug fix
- ✅ `v0.2.1-ground-truth-init-test.md` - test design and findings
- ✅ `v0.2.1-debug-logp-plan.md` - debugging methodology
- ✅ `tests/diagnostics/v0_2_1_divergence/README.md` - test suite guide
