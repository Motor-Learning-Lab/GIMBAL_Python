...
## 10. Post-Report Changes (2025-01-16)

Following the initial report, the following final alignment actions were taken to resolve open items:

1.  **Gamma Conversion Formula (Mismatch 6):**
    *   **Decision:** Align **code to spec** (contrary to initial report recommendation). The user directed that the exact formula from the spec be used instead of the approximation.
    *   **Action:** Updated `gimbal/prior_building.py` to use the exact formula: `rate = (mode + sqrt(mode^2 + 4*sd^2)) / (2*sd^2)`.
    *   **Verification:** Tests passed (see `results/v0.2.1_gamma_formula_verification.txt`).

2.  **Kappa Scale Semantics (Mismatch 4):**
    *   **Decision:** Confirmed code semantics (divisor). `kappa_scale` > 1 weakens the prior.
    *   **Action:** Updated `plans/v0.2.1_spec_data_driven_priors.md` to explicitly document this behavior ("higher = weaker").
    *   **Status:** **CLOSED**.

3.  **Invalid Joint Fallback (Mismatch 3):**
    *   **Decision:** Align spec to code.
    *   **Action:** Updated `plans/v0.2.1_spec_data_driven_priors.md` to state that invalid joints are excluded from configuration and handled via fallback in `hmm_directional.py`.
    *   **Status:** **CLOSED**.

**Final State:** The codebase and specification are now fully aligned. All mismatches have been resolved either by updating the spec or (in the case of the Gamma formula) updating the code.
