...
- K matrices: pixel-based (fx=fy=640, cx=640, cy=360) for image_size=[1280,720]
- All 4 datasets generated: dataset.npz + metrics.json + 5 figures each
...
- No bounds violations (K matrix fix successful)
- Jerk 95th percentile: ~13000-16000 (reasonable for skeleton/fps)

**Pending User Decisions:**
- Camera placement helpers + identifiability checking: implement now or defer to Step 4?
- Testing strategy: replace pytest thresholds with quality reports?
- Library organization: move metrics/visualization to gimbal/?
- Figure review: user validation of generated motion

**Note:** Items related to alignment, fitting toolchain, end-to-end runner, and camera priors are covered in separate Step documents.
...
* ⏳ **Camera quality validation:** Identifiability checks (awaiting implementation decision)
* ⏳ **Integration tests passing:** pytest strategy needs revision per user feedback
* ⏳ **Human validation completed:** Awaiting user (Opher) review of figures
...
- ✅ Camera quality: K matrices fixed, bounds violations resolved
- ✅ Comprehensive metrics and visualization: 5 figure types per dataset
...
- Add camera quality validation ⏳ (identifiability checks pending decision)
...
4. ⏳ A pytest integration test validates generator outputs (strategy under revision per user feedback)

**B. Human-in-the-loop acceptance (required):**

5. ⏳ Opher reviews the saved figures for L00–L03 and records approval note

**Current status:** Items 1-3 complete, item 4 needs strategy revision, item 5 awaiting user review.
