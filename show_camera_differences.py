...
for idx, t in enumerate(timesteps):
    ...
    bars1 = ax.bar(
        x_pos - width / 2, u_extents, width, label="u-extent (horizontal)", alpha=0.8
    )
    bars2 = ax.bar(
        x_pos + width / 2, v_extents, width, label="v-extent (vertical)", alpha=0.8
    )
    ...
    ax.set_title(
        f"t={t}, state={state} | 3D: X={x_extent_3d:.1f}, Y={y_extent_3d:.1f}, Z={z_extent_3d:.1f}",
        fontsize=12,
        fontweight="bold",
    )
    ax.set_xticks(x_pos)
...
plt.suptitle(
    "✅ NEW Camera System: Different Cameras = Different Views!\n"
