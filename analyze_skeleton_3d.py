...
for c in range(3):
    ...
    print(
        f"  Camera {c}: [{centers[c, 0]:.1f}, {centers[c, 1]:.1f}, {centers[c, 2]:.1f}], dist={dist:.1f}"
    )
...
print(
    f"  A 40-unit 3D extent should project to ~{40 * focal_length / distance:.1f} pixels"
)
print(
    f"  A 20-unit 3D extent should project to ~{20 * focal_length / distance:.1f} pixels"
)
